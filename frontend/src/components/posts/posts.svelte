<script>
  ...
  import { uploadImageStore, groupSelected, currentPosts, API_URL, IMAGE_URL } from "../../stores";
  import {
    getUserDetails,
    getPosts,
    selectUser,
  ...
  async function sendComment(postID, post) {
    const response = await fetch(`${API_URL}/newComment`, {
      credentials: "include",
      method: "POST",
      headers: {
            ...
            <div class="singlePost">
              ...
              <div class="postContent" on:click={() => toggleComments(index)}>
                <div class="content">
                  ...
                  <p><img src={IMAGE_URL}{post.img} alt={post.img} /></p>
                </div>
              </div>
